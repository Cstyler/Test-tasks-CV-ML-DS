...
def train(train_df_name, val_df_name, img_dir, model_num, initial_epoch, epochs, train_augment_prob, batch_size,
    ...
    train_gen = BatchGeneratorFreqModel(train_df, batch_size, images_dir_path, max_text_len,
    ...
    val_gen = BatchGeneratorFreqModel(val_df, batch_size, images_dir_path, max_text_len, shuffle=False)

    set_gpu(gpu_id)

    callbacks = get_callbacks(model_num, train_gen, val_gen)

    # TODO change this to model_lib config : dict str->value(float, sequence[float], etc.)=
